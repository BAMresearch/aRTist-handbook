...
* **ISR correlation length (mm)** to set the the ISR correlation length which describes the mean internal extension of the scattered radiation generated within the detector applying only for imaging plates and flat panel detectors. The ISR correlation length of the detector can be determined from an edge profile (:numref:`DigRadEdgeProfile`).

.. note::
    Two times the ISR correlation length gives the long range unsharpness of the aRTist detector model. 
